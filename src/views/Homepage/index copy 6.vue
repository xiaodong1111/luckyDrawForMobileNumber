...
function startLottery() {
    ...
    let whiteListWinners = whiteListhoneNumber.value.map((number, index) => ({
        phoneNumber: number,
        exchangeCode: whiteListexchange.value[index],
    }));

    intervalId = setInterval(() => {
        let availableWhiteListNumbers = whiteListWinners.filter(
            (winner) => !winners.value.some((w) => w.phoneNumber === winner.phoneNumber)
        );

        let randomIndex;
        if (availableWhiteListNumbers.length > 0) {
            randomIndex = Math.floor(Math.random() * availableWhiteListNumbers.length);
            selectedPhoneNumber.value = availableWhiteListNumbers[randomIndex].phoneNumber;
            selectedExchangeCode.value = availableWhiteListNumbers[randomIndex].exchangeCode;
        } else {
